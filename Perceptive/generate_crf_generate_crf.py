...
class CRFDataGenerator:
    ...
    def _build_index_maps(self):
        ...
        key_sub_indices     : {section_name: {obs_code: 0-based-index within same KEY group}}

        key_sub_indices explanation
        ---------------------------
        For checkbox observations sharing the same KEY (e.g. multiple rows all
        with KEY='issues'), OBS_INDEX gives the wrong bracket because it counts
        ALL observations in the section including non-checkbox ones.

        key_sub_indices counts ONLY among rows sharing the same KEY value,
        ordered by SEQUENCE. This gives issues[0], issues[1], issues[2]...
        regardless of where in the section those rows appear.

        Example:
          SEQUENCE  OBSERVATION_CODE          KEY       OBS_INDEX  KEY_SUB_INDEX
             1      IMAGE_QUALITY_ISSUES_FLAG hasIssues     0           0
             2      IMAGE_QUALITY_ISSUE_1     issues        1           0  <- resets to 0
             3      IMAGE_QUALITY_ISSUE_2     issues        2           1
             4      IMAGE_QUALITY_ISSUE_3     issues        3           2
             5      IMAGE_QUALITY_ISSUE_4     issues        4           3
             6      IMAGE_QUALITY_ISSUE_5     issues        5           4
             7      IMAGE_QUALITY_ISSUE_6     issues        6           5
             8      IMAGE_QUALITY_COMMENTS    imageQC       7           0

          IMAGE_QUALITY_ISSUE_1 -> bracket [0] -> issues[0].question  OK
          IMAGE_QUALITY_ISSUE_6 -> bracket [5] -> issues[5].question  OK
        """
        ...
        # Per-KEY sub-index: 0-based position within observations sharing the same KEY
        # This drives the correct bracket number for checkbox arrays
        key_sub_indices: Dict[str, Dict[str, int]] = {}
        for section_name, group in self.std_obs_df.groupby('SECTION_NAME'):
            sorted_obs  = group.sort_values('SEQUENCE')
            sec_key     = str(section_name).strip()
            key_sub_indices[sec_key] = {}
            key_counters: Dict[str, int] = {}
            for _, row in sorted_obs.iterrows():
                obs_code = str(row['OBSERVATION_CODE']).strip()
                key_val  = str(row.get('KEY', '')).strip() if pd.notna(row.get('KEY')) else ''
                if not key_val:
                    key_sub_indices[sec_key][obs_code] = 0
                    continue
                # Increment counter for this KEY value — resets per KEY group
                key_counters[key_val]              = key_counters.get(key_val, -1) + 1
                key_sub_indices[sec_key][obs_code] = key_counters[key_val]

        return crf_section_indices, obs_indices, key_sub_indices
    ...
    def generate_observations(self) -> pd.DataFrame:
        ...
        # Unpack all 3 index maps — including new key_sub_indices
        crf_section_indices, obs_indices_map, key_sub_indices_map = self._build_index_maps()
        results: List[dict] = []
        ...
        for _, crf_row in self.crf_sections_df.iterrows():
            ...
            for _, obs in obs_rows.iterrows():
                ...
                # KEY SUB-INDEX: for checkbox arrays, use the per-KEY counter
                # so that issues[0], issues[1]... are generated correctly
                # regardless of where in the section these rows appear.
                # For non-checkbox rows, falls back to obs_index.
                key_sub_index = key_sub_indices_map.get(section_name, {}).get(obs_code, obs_index)
                ...
                if is_checkbox:
                    # Use KEY_SUB_INDEX for the checkbox bracket — NOT obs_index
                    # This ensures issues[0], issues[1]... regardless of section position
                    checkbox_base = f"{full_pattern}[{key_sub_index}]"
